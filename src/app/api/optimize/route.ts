        ...
        const response = await fetch('http://47.99.87.50:3000/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'user',
                        content: `请优化以下内容的布局和文本，必须严格按照下面的规则。
                        1.为每个部分添加的标题（只能用h2-h5标签，如果原文不是需要改正,不要用markdonw格式，不要带#号），并且加上序号且序号不能重复！
                        2.图片大小和图片地址不变。
                        3.优化后的内容应采用专业的论文排版格式，每一段内容要紧凑。
                        4.首尾不要携带html标签。
                        5.<code>标签内的内容不要修改，如果代码没有换行那么一定要换行进行格式化。
                        6.对一些你觉得合适的部分可以加粗、斜体等做一些标记、若是地址转换成可跳转链接
                        7.标题格式一定要严格，同级标题要有相同的格式，不要出现不同级别标题格式相同的情况。
                        8.仅返回最终优化结果：${content}`
                    }
                ],
                temperature: 0.7
